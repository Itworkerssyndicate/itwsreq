...
async function submitRequest() {
    ...
    if(!n || nid.length !== 14) return Swal.fire("خطأ", "يرجى التأكد من الاسم والرقم القومي", "error");
    ...
    } catch(e) { Swal.fire("خطأ", "فشل في حفظ البيانات", "error"); }
    btn.disabled = false;
}
...
async function searchRequest() {
    ...
    if(snap.empty) return Swal.fire("خطأ", "بيانات الاستعلام غير صحيحة", "error");
