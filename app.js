...
async function submitRequest() {
    ...
    if(!n || nid.length !== 14) return Swal.fire("خطأ", "تأكد من الاسم والرقم القومي", "error");
    ...
    } catch(e) { Swal.fire("فشل", "حدث خطأ في الاتصال", "error"); }
...
async function searchRequest() {
    ...
    h += d.tracking.map(t=>`<div class="log-card"><b>${t.stage}</b><br><small>${t.comment}</small><br><small style="opacity:0.5">${t.date}</small></div>`).reverse().join('');
...
function loginAdmin() {
    ...
    } else Swal.fire("خطأ", "اليوزر أو الباسورد غلط", "error");
}
