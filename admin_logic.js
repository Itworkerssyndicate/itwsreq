...
function loadData(type) {
    ...
    // سحب الداتا بدون شروط معقدة لضمان الظهور
    db.collection("Requests").onSnapshot(snap => {
        let docs = [];
        snap.forEach(d => {
            let data = d.data();
            // فلترة بسيطة جداً حسب النوع (شكوى أو اقتراح)
            if(data.type === type) {
                docs.push({id: d.id, ...data});
            }
        });

        // الترتيب من الأحدث للأقدم يدوياً (عشان لو مفيش Index في فايربيز الداتا متختفيش)
        docs.sort((a, b) => {
        ...
        document.getElementById('tbody').innerHTML = h || "<tr><td colspan='5'>لا توجد بيانات حالياً</td></tr>";
    }, err => {
        console.error("Error fetching data: ", err);
        Swal.fire("خطأ في الاتصال", "تأكد من إعدادات Firebase وقواعد البيانات", "error");
    });
}
...
async function openCard(id) {
    ...
    let trackLine = `<div class="progress-container">
        <div class="progress-line"></div>
        <div class="progress-fill" style="width:${(idx/3)*100}%"></div>
        ${stages.map((s,i)=>`<div class="step-node ${i<=idx?'active':''}"><i class="fas fa-check"></i><span>${s}</span></div>`).join('')}
    </div>`;

    let logs = d.tracking.map(t => `<div class="log-card"><b>${t.stage}</b>${t.comment}<br><small>${t.date}</small></div>`).reverse().join('');

    Swal.fire({
        title: 'بيانات مقدم الطلب والتتبع',
        ...
        html: `
            <div style="text-align:right; font-size:12px;">
                ...
                <input id="new-stage" class="swal2-input" style="color:#000;" placeholder="اسم المرحلة القادمة">
                <textarea id="admin-rep" class="swal2-textarea" style="color:#000; height:60px;" placeholder="اكتب ردك هنا"></textarea>
                <button onclick="updateStep('${id}','تم الحل','تمت المعالجة وإغلاق الشكوى')" style="background:#10b981; color:#fff; border:none; padding:10px; width:100%; border-radius:5px; margin-bottom:15px; cursor:pointer;">إغلاق الشكوى (تم الحل)</button>
                <div style="max-height:120px; overflow-y:auto; border-top:1px solid #334155; padding-top:10px;">${logs}</div>
            </div>`,
...
async function deleteReq(id) {
    ...
    if(pass === '11111@') {
        ...
        Swal.fire("تم الحذف","تم مسح الطلب","success");
    } else if(pass) {
        Swal.fire("خطأ","الباسورد غير صحيح","error");
    }
}

// تشغيل الشكاوى أول ما نفتح الصفحة
loadData('شكوى');
